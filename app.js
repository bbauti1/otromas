...
const ComunicadoPreceptorResponsable = require('./models/ComunicadoPreceptorResponsable');
...
app.get('/responsable-dashboard', authenticateToken, async (req, res) => {
    ...
    try {
        const responsable = await Responsable.findById(req.user.userId);
        ...
        const comunicadosParaResponsable = await Comunicado.find({
            fk_id_responsable: req.user.userId
        });
        ...
        res.render('responsableDashboard', {
            ...
            comunicadosParaResponsable: comunicadosParaResponsable,
...
app.get('/directivo-dashboard', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        switch (directivo.estadoSolicitud) {
            ...
            case 'aceptado':
                return res.render('directivoDashboard', { directivo });
            default:
...
app.post('/enviar-comunicado', authenticateToken, async (req, res) => {
    if (req.user.role !== 'preceptor') {
        return res.status(403).send('Acceso denegado');
    }

    const { titulo, mensaje, estudianteId, responsableId } = req.body;

    try {
        const comunicado = new ComunicadoPreceptorResponsable({
            titulo,
            mensaje,
            fk_id_preceptor: req.user.userId,
            fk_id_estudiante: estudianteId,
            fk_id_responsable: responsableId
        });
        ...
        res.status(500).send('Error al enviar comunicado.');
    }
});

app.get('/comunicados-responsable', authenticateToken, async (req, res) => {
    if (req.user.role !== 'responsable') {
        return res.status(403).send('Acceso denegado');
    }

    try {
        const comunicados = await ComunicadoPreceptorResponsable.find({
            fk_id_responsable: req.user.userId
        }).populate('fk_id_estudiante'); 

        res.render('comunicadosResponsable', { comunicados });
    } catch (error) {
        console.error('Error al obtener los comunicados del responsable:', error);
        res.status(500).send('Error al cargar los comunicados');
    }
})

app.post('/marcar-leido-comunicado/:comunicadoId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'responsable') {
        return res.status(403).send('Acceso denegado');
    }
    ...
    try {
        const comunicado = await ComunicadoPreceptorResponsable.findById(comunicadoId);
        if (!comunicado.leido) {
            await ComunicadoPreceptorResponsable.findByIdAndUpdate(comunicadoId, { leido: true });
        }

        res.redirect('/comunicados-responsable');
    } catch (error) {
        console.error('Error al marcar comunicado como leído:', error);
        res.status(500).send('Error al marcar como leído el comunicado');
    }
});

app.get('/enviar-comunicado-responsable/:cursoId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'preceptor') {
        return res.status(403).send('Acceso denegado');
    }
    ...
    try {
        ...
        const responsables = await ResponsableDe.find({
            estadoSolicitud: 'aceptado'
        })
        .populate({
            path: 'fk_id_responsable',
            select: 'nombre apellido'
        })
        .populate({
            path: 'fk_id_estudiante',
            match: { cursoPerteneciente: cursoId },
            select: 'nombre apellido'
        });

        const curso = await Curso.findById(cursoId);

        const responsablesDelCurso = responsables.filter(rel => rel.fk_id_estudiante !== null);

        res.render('enviarComunicadoResponsable', {
            responsables: responsablesDelCurso,
            curso: curso
        });
    } catch (error) {
...
app.post('/enviar-comunicado-responsable/:cursoId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'preceptor') {
        return res.status(403).send('Acceso denegado');
    }

    const { responsableId, titulo, mensaje, estudianteId } = req.body;
    const cursoId = req.params.cursoId;

    try {
        const relacionResponsable = await ResponsableDe.findOne({
            fk_id_responsable: responsableId,
            'fk_id_estudiante': estudianteId, 
            estadoSolicitud: 'aceptado'
        });
        ...
        if (!relacionResponsable) {
            return res.status(404).send('Responsable no encontrado o sin estudiante asignado en este curso');
        }
        ...
        res.redirect(`/enviar-comunicado-responsable/${cursoId}`);
    } catch (error) {
...
app.get('/responsables-leidos-comunicados/:cursoId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'preceptor') {
        return res.status(403).send('Acceso denegado');
    }
    ...
    try {
        const comunicadosCurso = await ComunicadoPreceptorResponsable.find()
            .populate('fk_id_responsable')
            .populate('fk_id_estudiante');

        const comunicadosFiltradosPorCurso = comunicadosCurso.filter(comunicado => 
            comunicado.fk_id_estudiante.cursoPerteneciente.toString() === cursoId
        );

        const responsablesLeidos = await ComunicadoLeido.find({ leido: true })
            .populate('fk_id_responsable')
            .populate('fk_id_comunicado');

        const responsablesLeidosFiltrados = responsablesLeidos.filter(leido => {
            return comunicadosFiltradosPorCurso.some(comunicado => 
                comunicado._id.toString() === leido.fk_id_comunicado._id.toString()
            );
        });

        const responsablesDeCurso = await ResponsableDe.find({
            'fk_id_estudiante': { $exists: true }
        }).populate('fk_id_responsable fk_id_estudiante');

        const responsablesFiltrados = responsablesDeCurso.filter(relacion => {
            return relacion.fk_id_estudiante.cursoPerteneciente.toString() === cursoId;
        });

        res.render('responsablesLeidosComunicados', {
            comunicadosCurso: comunicadosFiltradosPorCurso,
            responsablesLeidos: responsablesLeidosFiltrados,
...
app.get('/comunicados', authenticateToken, async (req, res) => {
    try {
        ...
        const estudiante = await Estudiante.findById(req.user.userId).populate('cursoPerteneciente').exec();
        const comunicados = await Comunicado.find({
            $or: [
                { general: true },
                { curso: estudiante.cursoPerteneciente._id }
            ]
        }).populate('fk_id_preceptor').exec();

        res.render('verComunicados', { comunicados });
    } catch (error) {
